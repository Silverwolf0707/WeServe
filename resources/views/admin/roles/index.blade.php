...
.pr-id-cell { font-size: .74rem; font-weight: 700; color: var(--pr-sub); }
.pr-role-title {
    display: inline-flex; align-items: center; gap: 6px;
    font-size: .82rem; font-weight: 600; color: var(--pr-text);
}
.pr-role-dot { width: 6px; height: 6px; border-radius: 50%; background: var(--pr-lime-dim); flex-shrink: 0; }
...
/* hide DT default pagination/info */
...
.pr-pagination-info {
    font-size: .74rem; font-weight: 500; color: var(--pr-sub);
...
.pr-pagination-wrap .page-link {
    ...
    transition: background .15s, color .15s; cursor: pointer; background: var(--pr-surface);
    user-select: none;
}
...
.pr-pagination-jump {
    ...
    font-size: .74rem; font-weight: 500; color: var(--pr-sub); font-family: 'DM Sans', sans-serif;
}
...
<div class="pr-page">
    ...
    <div class="pr-hero">
        <div class="pr-hero-inner">
            <div class="pr-hero-left">
                <div class="pr-hero-icon"><i class="fas fa-users-cog"></i></div>
                <div>
                    <div class="pr-hero-title">{{ trans('cruds.role.title') }}</div>
            ...
            <div class="pr-hero-actions">
                @can('role_create')
                    <a class="pr-btn-primary" href="{{ route('admin.roles.create') }}">
    ...
    <div class="pr-table-card">
                ...
                <tbody>
                    @foreach($roles as $role)
                        <tr data-entry-id="{{ $role->id }}">
                            <td></td>
                            <td><span class="pr-id-cell">#{{ $role->id }}</span></td>
                            <td>
                                <span class="pr-role-title">
                                    <span class="pr-role-dot"></span>
                                    {{ $role->title }}
                                </span>
                            </td>
                            <td>
                                <div class="pr-action-wrap" style="justify-content:center;">
                                    @can('role_show')
                                        <a href="{{ route('admin.roles.show', $role->id) }}" title="View">
                                    ...
                                    @endcan
                                    @can('role_delete')
                                        <button type="button" class="del-btn delete-role-btn"
                                            data-id="{{ $role->id }}"
        ...
        <div id="pr-role-pagination" class="pr-pagination-wrap" style="display:none;">
            <div class="pr-pagination-info" id="pr-role-page-info"></div>
            <nav aria-label="Roles pagination">
                <ul class="pagination" id="pr-role-page-links"></ul>
            </nav>
            <div class="pr-pagination-jump">
                <span>Go to</span>
                <input type="number" id="pr-role-jump-input" min="1" placeholder="1" aria-label="Go to page">
                <button type="button" id="pr-role-jump-btn">Go</button>
            </div>
        </div>
    </div>
...
{{-- ══ DELETE MODALS ══ --}}
@can('role_delete')
    <div class="modal fade pr-modal" id="deleteRoleModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog"><div class="modal-content">
            ...
            <div class="modal-body">
                <p>Are you sure you want to delete the role <strong id="deleteRoleTitle"></strong>?</p>
            ...
            <div class="modal-footer">
                ...
                <form id="deleteRoleForm" method="POST" style="display:inline;">
                    @method('DELETE') @csrf
                    <button type="submit" class="btn btn-danger" id="confirmDeleteRoleBtn">
    ...
    <div class="modal fade pr-modal" id="massDeleteRoleModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog"><div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body">
                ...
                <div id="massDeleteRoleCount"
                    style="background:rgba(239,68,68,.08);border:1px solid rgba(239,68,68,.25);border-radius:8px;padding:10px 14px;font-size:.8rem;font-weight:600;color:#b91c1c;margin-top:8px;"></div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                <button type="button" class="btn btn-danger" id="confirmMassDeleteRoleBtn">
    ...
    $(function () {
        ...
        dtButtons.push({
            ...
            action: function (e, dt) {
                ...
                $('#massDeleteRoleCount').text(`You have selected ${ids.length} role(s) for deletion.`);
                const el = document.getElementById('massDeleteRoleModal');
            ...
            initComplete: function () {
                ...
                const searchWrap = $(`
                    <div class="pr-toolbar-search">
                        ...
                        <input type="text" id="roleSearchInput"
                               class="pr-toolbar-search-input"
                               placeholder="Search roles…"
                               autocomplete="off">
                        <button type="button" class="pr-toolbar-search-clear" id="roleSearchClear" title="Clear">
                ...
                /* ── Record indicator ── */
                const indicator = $(`
                    <div class="pr-record-indicator" id="pr-role-indicator">
                        <span class="ri-dot"></span>
                        <span class="ri-label">${total} role${total !== 1 ? 's' : ''}</span>
                    </div>
                `);
                ...
                /* ── Helpers ── */
                function setLabel(text) { $('#pr-role-indicator .ri-label').text(text); }
                function resetIndicator() {
                    $('#pr-role-indicator').css({ background: '', borderColor: '' });
                    $('#pr-role-indicator .ri-dot').css('background', '');
                }
                function activeIndicator() {
                    $('#pr-role-indicator').css({ background: 'rgba(6,78,59,.10)', borderColor: 'rgba(6,78,59,.35)' });
                    $('#pr-role-indicator .ri-dot').css('background', 'var(--pr-forest)');
                }
                ...
                function refreshPagination() {
                    ...
                    const cur       = info.page;
                    const last      = info.pages;
                    const totalRows = info.recordsDisplay;
                    const from      = totalRows === 0 ? 0 : info.start + 1;
                    const to        = info.end;
                    const wrap      = $('#pr-role-pagination');
                    const linksEl   = $('#pr-role-page-links');
                    const infoEl    = $('#pr-role-page-info');
                    const jumpInput = $('#pr-role-jump-input');

                    wrap.toggle(last > 0);
                    if (last === 0) return;

                    infoEl.html(`Showing <strong>${from}–${to}</strong> of <strong>${totalRows}</strong> roles`);
                    ...
                    pages.forEach(p => {
                        if (prev !== null && p - prev > 1) {
                            linksEl.append(`<li class="page-item disabled"><span class="page-link" style="border:none;background:transparent;padding:5px 4px;color:var(--pr-sub);">…</span></li>`);
                        }
                        linksEl.append(`
                ...
                $('#pr-role-jump-btn').on('click', function () {
                    const val  = parseInt($('#pr-role-jump-input').val());
                    const info = table.page.info();
                    if (!val || val < 1 || val > info.pages) { $('#pr-role-jump-input').focus(); return; }
                    table.page(val - 1).draw('page');
                    $('#pr-role-jump-input').val('');
                });
                $('#pr-role-jump-input').on('keydown', function (e) {
                    if (e.key === 'Enter') $('#pr-role-jump-btn').trigger('click');
                });

                table.on('draw', function () {
                    refreshPagination();
                    const searchVal = $('#roleSearchInput').val();
                    const info      = table.page.info();
                    if (searchVal) {
                        setLabel(`${info.recordsDisplay} of ${total} match${info.recordsDisplay !== 1 ? 'es' : ''}`);
                        activeIndicator();
                    } else if (table.rows({ selected: true }).count() === 0) {
                        setLabel(`${total} role${total !== 1 ? 's' : ''}`);
                        resetIndicator();
                    }
                });
                ...
                $('#roleSearchInput').on('input', function () {
                    ...
                    $('#roleSearchClear').toggleClass('visible', hasVal);
                });
                $('#roleSearchClear').on('click', function () {
                    $('#roleSearchInput').val('').trigger('input');
                });
                ...
                table.on('select deselect', function () {
                    ...
                    if (sel > 0) {
                        ...
                        const searchVal = $('#roleSearchInput').val();
                        if (searchVal) {
                            const matched = table.rows({ search: 'applied' }).count();
                            setLabel(`${matched} of ${total} match${matched !== 1 ? 'es' : ''}`);
                            activeIndicator();
                        } else {
                            setLabel(`${total} role${total !== 1 ? 's' : ''}`);
                            resetIndicator();
        ...
        $('#deleteRoleModal').on('hidden.bs.modal', () =>
            $('#confirmDeleteRoleBtn').prop('disabled', false).html('<i class="fas fa-trash-alt me-1"></i> Delete'));

        /* ── Mass delete ── */
        $('#confirmMassDeleteRoleBtn').on('click', function () {
            const btn = $(this);
            const ids = JSON.parse(document.getElementById('massDeleteRoleModal').dataset.selectedIds || '[]');
        ...
        $('#massDeleteRoleModal').on('hidden.bs.modal', () =>
            $('#confirmMassDeleteRoleBtn').prop('disabled', false).html('<i class="fas fa-trash-alt me-1"></i> Delete Selected'));
