...
.pr-detail-label {
    ...
    color: var(--pr-sub); margin-bottom: 4px; display: flex; align-items: center; gap: 5px;
}
.pr-detail-value {
...
.pr-detail-value.id-val { font-weight: 800; font-size: .95rem; color: var(--pr-forest); letter-spacing: .02em; }
...
.pr-btn-secondary {
    ...
    font-family: 'DM Sans', sans-serif; text-decoration: none; transition: background .18s; white-space: nowrap;
}
...
<div class="pr-hero">
    <div class="pr-hero-inner">
        <div class="pr-hero-left">
            ...
            <div>
                <div class="pr-hero-title">{{ trans('global.show') }} {{ trans('cruds.permission.title_singular') }}</div>
        ...
        <div class="pr-hero-actions">
            @can('permission_edit')
                <a href="{{ route('admin.permissions.edit', $permission->id) }}" class="pr-btn-primary">
        ...
        <div class="row g-4">
            <div class="col-md-3">
                <div class="pr-detail-field">
                    <div class="pr-detail-label"><i class="fas fa-hashtag"></i> {{ trans('cruds.permission.fields.id') }}</div>
                    <div class="pr-detail-value id-val">{{ $permission->id }}</div>
                </div>
            </div>
            <div class="col-md-9">
                <div class="pr-detail-field">
                    <div class="pr-detail-label"><i class="fas fa-key"></i> {{ trans('cruds.permission.fields.title') }}</div>
                    <div class="pr-detail-value">{{ $permission->title }}</div>
                </div>
            </div>
        </div>
    </div>
...
<div class="pr-action-footer">
    <a href="{{ route('admin.permissions.index') }}" class="pr-btn-secondary">
        <i class="fas fa-arrow-left" style="font-size:.78rem;"></i> {{ trans('global.back_to_list') }}
    </a>
    @can('permission_edit')
        <a href="{{ route('admin.permissions.edit', $permission->id) }}" class="pr-btn-edit">
            <i class="fas fa-edit" style="font-size:.78rem;"></i> Edit Permission
        </a>
    @endcan
</div>
