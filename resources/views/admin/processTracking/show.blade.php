...
<div class="card mb-4">
    ...
    <div class="card-body">
        <div class="row mb-4">
            <div class="col-md-6">
                <table class="table table-sm table-borderless">
                    ...
                    <tr><th>Date Processed:</th><td>{{ \Carbon\Carbon::parse($patient->date_processed)->format('F j, Y g:i A') }}</td></tr>
                    <tr><th>Claimant Name:</th><td>{{ $patient->claimant_name }}</td></tr>
                    <tr><th>Case Worker:</th><td>{{ $patient->case_worker }}</td></tr>
                    <tr><th>Status:</th><td>{{ $latestStatus->status }}</td></tr>
                </table>
            </div>
        </div>
                ...
                <ul class="list-group">
                    @foreach ($patient->statusLogs as $log)
                        <li class="list-group-item">
                            <strong>{{ ucfirst($log->status) }}:</strong> {{ $log->user->name ?? 'System' }} -
        ...
        <div class="form-group mt-4">
            <a class="btn btn-secondary" href="{{ route('admin.document-management.index') }}">
