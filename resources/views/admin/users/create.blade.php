...
.pr-hero::before {
    ...
    background: radial-gradient(ellipse 380px 200px at 95% 50%, rgba(116,255,112,.13) 0%, transparent 65%),
...
.pr-input-group { display: flex; gap: 0; }
.pr-input-group .pr-input { border-radius: 8px 0 0 8px; border-right: none; flex: 1; }
.pr-input-group .pr-input:focus { z-index: 1; }
.pr-toggle-btn { display: flex; align-items: center; justify-content: center; width: 42px; border: 1.5px solid var(--pr-border-dark); border-left: none; border-radius: 0 8px 8px 0; background: var(--pr-surface2); color: var(--pr-sub); cursor: pointer; transition: background .15s, color .15s; font-size: .78rem; flex-shrink: 0; }
...
<div class="pr-hero">
    <div class="pr-hero-inner">
        <div class="pr-hero-left">
            ...
            <div>
                <div class="pr-hero-title">{{ trans('global.create') }} {{ trans('cruds.user.title_singular') }}</div>
                <div class="pr-hero-sub">Fill in the details to create a new system user account.</div>
            </div>
        </div>
...
<form method="POST" action="{{ route('admin.users.store') }}" enctype="multipart/form-data" id="createUserForm">
    ...
    <div class="pr-form-card">
        ...
        <div class="pr-form-body">
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="name" id="name" class="pr-input {{ $errors->has('name') ? 'is-invalid' : '' }}" value="{{ old('name', '') }}" placeholder="Full name" required autofocus>
                    <div class="pr-hint"><i class="fas fa-info-circle"></i> {{ trans('cruds.user.fields.name_helper') }}</div>
                ...
                <div class="col-md-6">
                    ...
                    <input type="email" name="email" id="email" class="pr-input {{ $errors->has('email') ? 'is-invalid' : '' }}" value="{{ old('email') }}" placeholder="user@example.com" required>
                    <div class="pr-hint"><i class="fas fa-info-circle"></i> {{ trans('cruds.user.fields.email_helper') }}</div>
                    @error('email') <div class="pr-error"><i class="fas fa-exclamation-circle me-1"></i>{{ $message }}</div> @enderror
                </div>
                <div class="col-md-6">
                    <label class="pr-label" for="password">{{ trans('cruds.user.fields.password') }} <span class="req">*</span></label>
                    <div class="pr-input-group">
                        <input type="password" name="password" id="password" class="pr-input {{ $errors->has('password') ? 'is-invalid' : '' }}" placeholder="Minimum 8 characters" required>
                        <button type="button" class="pr-toggle-btn" id="togglePasswordBtn" tabindex="-1">
                            <i class="fas fa-eye" id="togglePasswordIcon"></i>
                        </button>
                    </div>
                    <div class="pr-hint"><i class="fas fa-info-circle"></i> {{ trans('cruds.user.fields.password_helper') }}</div>
        ...
        <div class="pr-form-body">
            ...
            <div class="pr-hint" style="margin-top:8px;"><i class="fas fa-info-circle"></i> {{ trans('cruds.user.fields.roles_helper') }}</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateCount() {
        const c = $('#roles').val()?.length ?? 0;
        document.getElementById('roleCountBadge').textContent = `${c} selected`;
    }
    ...
    const fi = document.querySelector('.pr-input.is-invalid');
    if (fi) fi.focus();
});
</script>
@endsection